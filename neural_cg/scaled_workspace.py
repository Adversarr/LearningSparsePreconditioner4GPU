...
from neural_cg.utils.validate import (
    ...
    get_pcg_scaled_iter_time_scipy,
    get_pcg_stat_scipy,
    to_csr_cpu,
    to_numpy,
)
...
class ScaledTrainingWorkspace(L.LightningModule):
    def __init__(
        self,
        # Trainer
        batch_size: int,
    ...
    def _shared_step(self, batch: Data, prefix: str, batch_idx: int) -> torch.Tensor:
        ...
        assert hasattr(batch, "inv_diag"), "Relying on batch.inv_diag to maintain stability"
        bsize = batch.ptr.shape[0] - 1
        boo_entry = self(batch.x, batch.edge_index, batch.edge_attr)
        d = self.spai_preconditioner(batch.residual, batch.edge_index, boo_entry, mask=batch.mask, diag=batch.inv_diag)
    ...
    def log_converge_batch(self, boo_entry: torch.Tensor, batch: Data, prefix: str, repeat=1):
        ...
        rsqrt_diag = to_numpy(batch.rsqrt_diag).flatten().astype(np.float64, copy=True)
        L = L @ diags(rsqrt_diag)  # type: ignore
    ...
    def log_converge_v2(self, L, A, r, repeat, prefix: str):
        ...
        for device in self.check_devices:
            ...
            avg_iter, _, avg_solve = get_pcg_scaled_iter_time(A, r, L, self.epsilon, device=device, repeat=repeat)
    ...
    def log_converge_scipy(self, L, A, r, repeat, prefix: str):
        ...
        stats[f"{prefix}/cpu_neural_iter"] = get_pcg_scaled_iter_time_scipy(A, r, L, self.epsilon)
        self.log_dict(stats, batch_size=1)
    ...
    def inference_step(self, sample: Data, time_beg: Union[float, None] = None):
        ...
        rsqrt_diag = to_numpy(sample.rsqrt_diag).flatten().astype(np.float64, copy=True)
        csr = csr_matrix(csr @ diags(rsqrt_diag))  # type: ignore
        return csr, dt


    def precondition(self, sample: Data, Adense) -> np.ndarray:
        L, _ = self.inference_step(sample)
        diags = np.diag(1.0 / np.diag(Adense))
        M = L @ diags @ L.T + self.epsilon * diags
